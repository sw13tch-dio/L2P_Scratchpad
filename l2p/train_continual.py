...
class TrainState:
  ...
  params: Any
  opt_state: Any
  optimizer: Any  # optax chain, for reinit and updates
  model_state: Any


def _make_freeze_mask(params: Any, freeze_part: List[str]) -> Any:
  """Build a pytree mask: True = freeze, False = train."""
  def _leaf(path, _):
    path_str = str(path)
    return any(f in path_str for f in freeze_part)
  return jax.tree_util.tree_map_with_path(_leaf, params)


def create_optimizer(config: ml_collections.ConfigDict, params: Any):
  """Creates optax optimizer and initial state.

  Args:
    config: Configuration to use.
    params: Parameters for initializing optimizer state.

  Returns:
    Tuple of (optimizer chain, opt_state).
  """
  wd = config.weight_decay
  lr_scale = 1.0  # LR applied per-step in train_step

  if config.optim in ("adamw", "adam"):
    if config.get("optim_wd_ignore"):
      raise NotImplementedError("optim_wd_ignore requires optax.masked;"
                                " use freeze_part for now.")
    base_opt = optax.adamw(learning_rate=lr_scale, weight_decay=wd)
  ...
  if (not config.get("freeze_part")) or config.get("optim_wd_ignore"):
    ...
    mask = _make_freeze_mask(params, config.freeze_part)
    try:
      chain = optax.chain(base_opt, optax.transforms.freeze(mask))
    except (AttributeError, TypeError):
      chain = optax.chain(base_opt, optax.masked(optax.set_to_zero(), mask))

  opt_state = chain.init(params)
  return chain, opt_state
...
def create_train_state(config: ml_collections.ConfigDict, rng: np.ndarray,
  ...
  return model, TrainState(
      ...
      optimizer=optimizer, model_state=model_state)
...
def train_step(
  ...
  (loss, (new_model_state, logits)), grad = grad_fn(state.params)
  ...
  updates = jax.tree_map(lambda u: lr * u, updates)
...
def train_and_evaluate_per_task(task_id: int, config: ml_collections.ConfigDict,
  ...
  if task_id > 0 and config.reinit_optimizer:
    optimizer, opt_state = create_optimizer(config, state.params)
    state = state.replace(optimizer=optimizer, opt_state=opt_state)
    if config.continual.get("weights_transfer"):
      param_dict = state.params
      transferred_weights = utils.transfer_weights(
          config,
          param_dict,
          task_id,
          kernel_only=config.continual.get("kernel_only"))
      state = state.replace(params=transferred_weights)
      opt_state = optimizer.init(transferred_weights)
      ...
      if config.get("no_train") and in_train_session:
        ...
        if config.get("review_last_only") and in_review_session and task_id < (
          ...
          if config.get("weight_norm"):
            ...
            param_dict = utils.weight_norm(state.params)
            state = state.replace(params=param_dict)
            state = flax_utils.replicate(state)
