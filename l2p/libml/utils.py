...
def _load_and_custom_init_checkpoint(init_state: Any,
  ...
  # 1) Copy model params - support both old (optimizer.target) and new format
  state = checkpoint.load_state_dict(os.path.split(checkpoint_path)[0])
  if "params" in state:
    init_param_dict = state["params"]
  else:
    init_param_dict = state["optimizer"]["target"]
  init_param_dict = flax.core.unfreeze(init_param_dict)
  ...
  if resize_posembed != 1:
    ...
    for pkey in init_param_dict.keys():
      ...
      if POS_EMBED in pkey:
        ...
        pa_expected_shape = init_params[pkey][var_name].shape
      ...
      if HEAD in pkey:
        ...
        for var_name in var_names:
          ...
          if reinit_head == "zero_all":
            pa_new = jnp.zeros_like(init_params[pkey][var_name])
          else:
  ...
  # 4): Copy model params to init_state, reinit opt_state
  restored_params = flax.core.freeze(init_param_dict)
  opt_state = init_state.optimizer.init(restored_params)
  init_state = init_state.replace(params=restored_params, opt_state=opt_state)
  return init_state
...
def load_and_custom_init_checkpoint(*, config: ml_collections.ConfigDict,
  ...
  if model_type.startswith("ViT"):
    ...
    loaded_param_dict = (
        state["params"]
        if "params" in state
        else state["optimizer"]["target"])
    loaded_param_dict = flax.core.unfreeze(loaded_param_dict)
    loaded_model_state = state.get("model_state", {})
    # we should always change the classification head
    loaded_param_dict["head"]["kernel"] = init_state.params["head"]["kernel"]
    loaded_param_dict["head"]["bias"] = init_state.params["head"]["bias"]
    loaded_param_dict = flax.core.freeze(loaded_param_dict)
    opt_state = init_state.optimizer.init(loaded_param_dict)
    init_state = init_state.replace(
        params=loaded_param_dict, opt_state=opt_state,
...
def state_with_new_param(state, param_dict):
  ...
  opt_state = state.optimizer.init(param_dict)
  state = state.replace(params=param_dict, opt_state=opt_state)
  return state
