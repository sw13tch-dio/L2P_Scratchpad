...
def split_single_dataset(dataset_train, dataset_val, args):
    ...
    for _ in range(args.num_tasks):
        ...
        # Subsample training data if requested (e.g. --subsample 0.1 for 10%)
        if hasattr(args, 'subsample') and args.subsample < 1.0:
            n = max(1, int(len(train_split_indices) * args.subsample))
            train_split_indices = train_split_indices[:n]
