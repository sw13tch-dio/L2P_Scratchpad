...
These three cells run the original Google Research L2P code on modern JAX/Flax.
Tested on Colab T4 GPU with High RAM.
- Task 1: 97.8% accuracy (27.4min)
- Task 2: 95.4% avg accuracy, 2.6% forgetting (30.8min)

**Instructions**: Fresh kernel → Cell 1a → Cell 1b → Cell 2. That's it.

---

## Cell 1a — Install, clone, patch source files
...
# Patch: jax.tree_map/leaves/flatten (removed in JAX 0.6+)
...
# Patch: add unfreeze to train_continual.py (FrozenDict vs optax)
...
# Patch: add unfreeze to utils.py
...
## Cell 1b — Patch checkpoints (must be its own cell because of %%writefile)

```python
%%writefile /tmp/patch_ckpt.py
import re
with open('/tmp/l2p-jax/train_continual.py') as f:
    t = f.read()
t = re.sub(r'    ckpt = checkpoint\.MultihostCheckpoint\([^)]+\)', '    pass  # ckpt disabled', t)
t = re.sub(r'    state = ckpt\.restore_or_initialize\(state\)', '    # ckpt restore disabled', t)
t = re.sub(r'              ckpt\.save\([^)]+\)', '              pass  # ckpt save disabled', t)
with open('/tmp/l2p-jax/train_continual.py', 'w') as f:
    f.write(t)
print('checkpoints patched')
```

```python
!python /tmp/patch_ckpt.py
...
## Cell 2 — Shims + Run (after both Cell 1a and 1b show ✅)
    ...
    def apply_gradient(self, grad, learning_rate=None, hyper_params=None):
        ...
        if hyper_params is not None and self.param_labels is not None:
            lr_tree = jax.tree.map(
                lambda l: hyper_params[l].learning_rate, self.param_labels)
        ...
        return OptaxOptimizer(self.tx,
            optax.apply_updates(self.target, updates),
...
jax.tree_util.register_pytree_node(OptaxOptimizer,
    lambda o: ((o.target, o.opt_state),
               (o.tx, o.optimizer_def, o.param_labels)),
    lambda aux, ch: OptaxOptimizer(
        aux[0], ch[0], ch[1], aux[1], aux[2]))
class _Adam:
    def __init__(self, weight_decay=0, learning_rate=None):
        self.wd = weight_decay
    def _make_tx(self):
        return optax.chain(optax.scale_by_adam(),
            optax.add_decayed_weights(weight_decay=self.wd),
            optax.scale(-1.0))
    def create(self, p):
        tx = self._make_tx()
        return OptaxOptimizer(tx, p, tx.init(p))
class _Mom:
    ...
    def _make_tx(self):
        ...
        return optax.chain(
            optax.trace(decay=self.beta, nesterov=self.nesterov),
            optax.scale(-1.0))
    def create(self, p):
        tx = self._make_tx()
        return OptaxOptimizer(tx, p, tx.init(p))
...
class _MO:
    ...
    def create(self, params):
        transforms = {i: od._make_tx()
                      for i, (_, od) in enumerate(self.groups)}
        def lf(path, leaf):
            ps = _pts(path)
...
m.Optimizer = OptaxOptimizer
m.Adam, m.Momentum = _Adam, _Mom
m.ModelParamTraversal, m.MultiOptimizer = _MPT, _MO
flax.optim = m
sys.modules['flax.optim'] = m
...
def _rot(img, rad):
    ...
    return tf.raw_ops.ImageProjectiveTransformV3(
        ...
        transforms=tf.expand_dims(
            tf.cast([c,-s,0,s,c,0,0,0], tf.float32), 0),
        output_shape=tf.shape(img)[:2],
        interpolation="BILINEAR",
        fill_mode="NEAREST", fill_value=0.0)[0]
def _trans(img, t):
    return tf.raw_ops.ImageProjectiveTransformV3(
        images=tf.expand_dims(img, 0),
        transforms=tf.expand_dims(
            tf.cast([1,0,-t[0],0,1,-t[1],0,0], tf.float32), 0),
        output_shape=tf.shape(img)[:2],
        interpolation="BILINEAR",
        fill_mode="NEAREST", fill_value=0.0)[0]
def _xform(img, t, **kw):
...
print(f"⚙️  coeff={config.pull_constraint_coeff} "
      f"bs={config.per_device_batch_size} "
...
def run():
    ...
    for tid in range(config.continual.num_tasks):
        ...
        print(f"✅ Task {tid+1}: {te/60:.1f}min | "
              f"avg_acc={avg*100:.1f}% | "
              f"total={tt/60:.1f}min")
        if tid > 0:
    ...
    print(f"\n{'='*60}")
    print(f"🏆 FINAL AVG ACCURACY: {final*100:.1f}% "
...
- **Cell 1b uses `%%writefile`** which MUST be the first line in its cell
- **Task 1 takes ~27min** (JIT compilation overhead). Tasks 2+ are ~15-30min each.
- **Total runtime**: ~3-4 hours on T4, ~1-2 hours on A100
- **Must restart kernel** if you re-run Cell 1a (clears cached failed imports)
- Checkpointing is disabled — if runtime disconnects, restart from scratch
- All `WARNING:absl` messages about `get_read_instruction_for_host` are harmless
- GPU RAM usage: ~11.4/15GB on T4 — normal for ViT-B/16 + prompt pool
